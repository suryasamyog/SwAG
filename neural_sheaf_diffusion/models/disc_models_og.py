...
class DiscreteDiagSheafDiffusion(SheafDiffusion):
    ...
    def __init__(self, edge_index, args):
        ...
        # EDITS
        # --- ADDED NORMALIZATION ---
        if args.get("norm") == "group":
            # Note: Collab uses num_groups=self.final_d for Diag/Gen, but args["hidden_channels"] for Bundle
            self.gn = nn.GroupNorm(num_groups=self.final_d, num_channels=self.hidden_dim)
            self.bn = None
        elif args.get("norm") == "batch":
    ...
    def forward(self, x):
        ...
        # EDIT
        if self.use_act:
            x = self.act(x)  # Changed from F.elu(x)
